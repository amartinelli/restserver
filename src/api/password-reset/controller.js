...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your ContribusServer account.<br>
        ...
        &mdash; ContribusServer Team
      `
      return sendMail({ toEmail: email, subject: 'ContribusServer - Password Reset', content })
    })
